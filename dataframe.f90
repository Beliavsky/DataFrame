...
type :: DataFrame
   ...
   contains
      procedure :: read_csv, display=>display_data, write_csv, irow, icol, &
         loc, append_col, append_cols, set_col, reindex, shift, pct_change
end type DataFrame
...
! shift values by periods along the row axis, keeping the same index/columns.
! periods > 0 shifts values down; periods < 0 shifts values up.
...
df_new%index = self%index
df_new%columns = self%columns
allocate(df_new%values(nrow(self), ncol(self)))
if (nrow(self) == 0 .or. ncol(self) == 0) return
...
method_ = adjustl(default("none", method))
fill = default(ieee_value(0.0_dp, ieee_quiet_nan), fill_value)
